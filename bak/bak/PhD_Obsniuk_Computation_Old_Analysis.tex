...
The task analysis, the thematic analysis (excel sheets and thematic maps).

Come back to the task analysis in chapter 5.  A section that describes data reduction: how we looked at the data, when decisions were made to pull out data from the task analysis.  Point back to section 2.4.  Have an example:  what were the markers that we used to decide that an expert was relevant to our task analysis.
...
Accordingly, the group can be seen going back through their code and systematically checking for variables that can be turned to vectors: \begin{description}
\SB: it needs to be a vector    
\SC: yeah it should be vector type but it wasnt...
\SC so the problem is, this one is vector [D], this one is not a vector [m]
\SC the problem is not av ector and vector, we trying to combine together
\SC we never gonna get answer as computer cause its a computer\end{description}  Mistakenly, SC believes that in order for the program to be able to calculate their net force, they need to turn every other variable used in the calculation of the net force into a vector (i.e., the mass of the satellite and the separation distance).

Thus, Group D can be seen not only identifying an unexpected error in their program, but they can be seen to systematically work toward rectifying that error by turning each variable used in the calculation of the net force into a vector.  Although this group is later encouraged by the TA to change some of the variables back to scalars, it is a clear example of a systematic process of troubleshooting and debugging.

\subsection{Creating computational abstractions}
