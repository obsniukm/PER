...
What the group was doing before the transcript.

What the group was doing after the transcript.

What actions and inferences occurred in the transcript.

The rationale according to the framework.

The rationale within the excerpt.

The rationale beyond the excerpt.

Initial confidence.

Inter-rater suggestions.
...
Consider Excerpt 4 from Group D.  The following transcript (occurring at around one hour into the groups work and occurring over roughly 2 minutes) shows the group working towards defining a function in the parameters/initial conditions of their MWP: \begin{description}
\SB \textit{(104)} yeah			
\SC \textit{(105)} its like theyve just given {these values}...		
...
\SB \textit{(119)} thats a good idea			
...
\SB \textit{(124)} you calculated that out? {the force}			
\SD \textit{(125)} uhhh	
\SA \textit{(126)} thats what i have is the force, yeah				
\SB \textit{(127)} so should i put \textbf{a parameter} in or...			
\SA \textit{(128)} i dunno				
\SD \textit{(129)} position of satellite...	
\SC \textit{(130)} do they have a position of the earth		
\SC \textit{(131)} and a position of the satellite?		
\SB \textit{(132)} yeah here [points to code]			
\SA \textit{(133)} so its saying the earth is at the origin...			
\end{description}

Before this portion of transcript, the group was engaging in the practice of assessing a computational model.  They talk about how they were given dummy values on the last computational problem, and were required to modify those dummy values so that they reflected the ralistic situation.  This leads into the group analyzing the values that were provided for the mass of the satellite and the position of the satellite in line (105).
...
Initially this excerpt started off as being classified, according to the framework, as both creating data and creating computational abstractions, with medium and low confidence, respectively.  The practice that ended up being more confidently identified through inter-rater reliability was that of creating abstractions.  This is because the characteristics identified in the excerpt align more with the characteristics of the framework for creating computational abstractions.  Specifically, the group is: identifying computational abstractions, creating computational abstractions, and using a computational abstraction as they work toward a goal.
...
Beyond the excerpt, we eventually see the group using their computational abstraction of the position of the satellite to construct the Newtonian gravitational force.

Initially, the confidence on the classification of this excerpt as creating computational abstractions was low.  However, through the process of inter-rater reliability, the confidence on creating data was lowered and the confidence on computational abstractions was raised to high.  The final and high confidence in classifying this excerpt as the computational practice of creating abstractions hinges on the group, specifically SD in line (118), suggesting that they construct a function for the program to calculate a set of numerical data -- as opposed to calculating each individual force by hand like SA is in line (108).

After this portion of the transcript, the group ends up engaging in the practice of creating data.  The data that they end up creating are the various force values as the satellite moves around the Earth.  This set of data helps them to better visualize the situation at hand.
