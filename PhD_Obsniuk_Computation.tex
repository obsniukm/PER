...
The computational practice of creating abstractions, as defined by Weintrop et. al, requires ``the ability to conceptualize and then represent an idea or a process in more general terms.''  This means that we need to observe three fundamental characteristics within an excerpt: i) identifying and ii) creating computational abstractions, and iii) working toward a goal.  These three characteristics, if confidently observed within an excerpt, would serve to classify that excerpt as creating computational abstractions.

Consider Excerpt 13 from Group D in the following analysis.  Overall, the group can be seen identifying a computational abstration, beginning to create that computational abstraction, and planning to use that abstraction as they work toward a goal.  They start by looking for an equation that they can use to try to calculate the net force on the satellite: \begin{description}
...
\SB \textit{(119)} thats a good idea\end{description} Although they have clearly identified that $\hat{r}$ is an abstraction that they can use to construct the force on the satellite, they are a little stuck on how to actually go about creating it.  However, they at least know that they want it to be a function (line 118) rather than just a constant numerical value.  Presumably, this is because they know that the numerical values will need to change in time (line 271):\begin{description}
...
To summarize, the computational abstraction that the group has \textit{identified} is the unit vector for the position of the satellite (line 109).  They plan to \textit{create} this computational abstraction by first deciding to place it (incorrectly) in the initial conditions/parameters section of the MWP (line 117).  Eventually, the group does correctly create the the unit vector, and uses that computational abstraction to work toward the \textit{goal} of constructing the Newtonian gravitational force on the satellite.  Thus, the \textit{idea/process} that the group is working with is that of a non-constant position-position dependent net force, and the \textit{general term} that the group represents this idea/process with is the unit vector ($\hat{r}$).

\subsection{Troubleshooting and debugging}

The computational practice of troubleshooting and debugging, as broadly defined by Weintrop et. al, refers to ``the process of figuring out why something is not working or behaving as expected.''  This process is frequently undertaken by students in all fields of study -- especially within introductory computational physics, given the reliance on incomplete/approximate computational and physical models.  The five fundamental characteristics of this computational practice are: i) identifying, ii) isolating, iii) reproducing, and ultimately iv) correcting unexpected errors while doing so v) in a systematic way.  These five characteristics, if confidently observed within an excerpt (either in totality or only partially), would serve to classify that excerpt as troubleshooting and debugging.

For example, consider Excerpt 2 from Group I in the following analysis.  The group begins by reading through the Euler-Cromer update of the position of the satellite in the calculation loop (line 6).  This update involves the position of the satellite, the momentum of the satellite, the mass of the satellite, and the discrete time step (i.e., \texttt{satellite.pos = satellite.pos + satellite.p/msatellite*dt}):  \begin{description}
...
\end{description}  By changing the mass of the satellite from \texttt{1} to \texttt{1.5E4} (line 12), they have correctly modified the program to reflect the realistic situation presented to them.  However, by changing the mass of the satellite they have also introduced an unexpected error.
...
\SA \textit{(20)} uhh its not moving, maybe we should...\end{description}  Given this unexpected error, the group begins to identify/isolate/reproduce the cause of the unexpected error.  They consider that they may have introduced a syntax error since they last ran the program (e.g., in using E as opposed to **), resulting in it crashing the program (line 22).  They also consider that changing the mass might have lead to the unexpected error, and work to at least temporarily rectify it (line 25): \begin{description}
...
To summarize, the unexpected error that the group runs into is that in changing the mass of the satellite to reflect the realistic situation, the satellite remains motationless relative to the rotating Earth (line 20).  This introduces concern to the group, presumably because a straight line trajectory is closer to a geostationary orbit as compared to no trajectory at all.  The group identifies/isolates/reproduces the error by changing the mass of the satellite back to its initial dummy value and finding that this does indeed rectify the unexpected error (line 25).  Ultimately, the group works to correct this error by redinfing the momentum of the satellite as a function (line 745).
