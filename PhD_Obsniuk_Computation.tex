...
In the sections that follow, we present our findings of the common, less common, and unobserved practices within our data set.  The common practices are those that were identified at least three times in a majority of the groups (individual practices occurred between zero and seven times per group, with an average of three occurrences).  The less common practices are the remaining of the observed practices.  The unobserved practices were not identified at all.  Along with some of the statistics of the frequencies of these practices (i.e., raw numbers and percentages), we provide their definitions and reference detailed examples.  Additionally, we discuss why a particular practice might have a given frequency.
...
Eleven of the practices laid out by the framework have been identified multiple times in almost every group that was analyzed.  Accordingly, these practices (listed in Tab.~\ref{CH6:Common}) are deemed common and are discussed below.  It is important to pay attention to these practices because, as instructors, we not only want students to accomplish tasks, but we also want to make sure that they are engaging in things like critical and computational thinking.  Being able to identify and encourage these practices as they occur (or don't occur) in a classroom, therefore, is crucial to effective pedagogy and course design.  We must develop clear and reliable definitions for each of the common practices.
...
The practice of \textbf{visualizing data} involves the \textit{production} of a visualization that clearly \textit{conveys} some information.  For example, the computational production of a dynamically updating graph of the distance between the satellite and the Earth vs. simulated time can be used to clearly convey information about the nature of the orbit (i.e., how close the orbit is to perfectly circular).  This type of practice was observed thirty-six times across the data set, accounting for $\%$ of the data practices, and $\%$ of all practices.

Visualizing data is expected to show up commonly in our data given that students have been working with MWPs to produce dynamic visualizations of motion since the first week of class.  The first and second computational problems, focusing on boats and hovercrafts, respectively, were visualized in a number of ways (e.g., producing visualizations of their trajectories or graphs of velocity vs. time).  In other words, students are familiar with the visualization of data coming into the third problem.  Furthermore, the problem statement, shown in Fig.~\ref{CH3:SatelliteProblem}, explicitly asks students to generate a simulation/visualization of an elliptical orbit. 

Additionally, the tutor questions -- the ones asked after the group has correctly constructed the Newtonian gravitational force -- invariably focus on the generation of a 2D graph to clearly show the distance between the Earth and satellite over time.  For example, consider Excerpt X from Group Y: \begin{description}
...
Above all, we want students in ${\rm P}^{3}$ to understand the difference between time-dependent and time-independent physical quantities, and to be able to properly define and place them in code.
...
Five of the practices laid out by the framework have been identified just a couple of times and in only some of the groups that we analyzed.  Accordingly, these practices (listed in Tab.~\ref{CH6:LessCommon}) are deemed less common and are given a reasonable amount of attention.
...
\end{tabular}
\caption{The computational practices that have been deemed less common are shown with the number of times each practice was identified, the percentage of its category that it occupies, and the percentage of all the practices that it occupies.}\label{CH6:LessCommon}
\end{table}

The practice of \textbf{understanding relationships} in a system involves \textit{identifying} the individual elements of the system and \textit{explaining} their relationships to one another.  For example, a group might identify the mass and local acceleration due to gravity as the individual elements of a system.  The group could then explain the relationship between these two elements as they relate to the local gravitational force (i.e., the mass and acceleration are multiplied to produce the force).  This type of practice was observed thirteen times across the data, accounting for $\%$ of the systems thinking practices, and $\%$ of all practices.

Understanding relationships most likely shows up in our data given the learning goal of being able to develop models (P1).  All of the computational force models that must be developed in code depend on individual elements.  These individual elements are related to one another by standard mathematical operations (e.g., by an addition or multiplication) to produce expected behavior.  Additionally, groups are frequently asked to explain their thought process, inspired by the learning goal of being able to construct explanations (P4).

Further, the course is designed to cover multiple force models with widely varying complexity.  For example, the first week focuses on a constant velocity motion, with a constant zero force model.  The second week focuses on constant acceleration motion, with a constant local gravitational force model.  The third week focuses on non-constant forces, like the centripetal force and the Newtonian gravitational force.  Given the complexity of the models used in the third week, we expect to see students attempting to understand them and to explain that understanding.  Groups often engage in this explanation either spontaneously or as requested from a TA.  For example, consider Excerpt X from Group Y: \begin{description}
\TA I'm still confused by that line of code...
\TA You gotta explain that {force} to me.
\end{description}  This type of tutor question often prompts long periods of explanation and discussion.
...
The practice of \textit{investigating systems} as a whole involves \textit{questioning} and \textit{interpreting} data gathered from a system.  For example, the set of data representing the position of the satellite can be questioned and interpreted without knowing the underlying details of the system and/or model.  This type of practice was observed thirteen times across the data, accounting for $\%$ of the systems thinking practices, and $\%$ of all practices.

Investigating systems as a whole is expected to show up in our data given the learning goals of planning and carrying out investigations (P2) and analyzing and interpreting data (P3).  The act of questioning what you don't know and investigating for answers is scaffolded by the facts and lacking quadrants of the four-quadrants.  Similarly, the large sets of data that can be generated by computation are easily visualized, analyzed, and interpreted.

Additionally, the course was designed so that students would not need to engage with the underlying system of the program (i.e., with the Visual or PhysUtil modules).  Rather, they were encouraged to engage with the various force models and to analyze and visualize the resulting data.  This aligns well with the practice of investigating systems as a whole.  Further, many tutor questions focus on the interpretation of data.  For example, consider Excerpt X from Group Y:  \begin{description}
...
\end{description}  Here, the group is being asked to question the meaning of the sinusoidal data that they have visualized graphically.
...
Eight of the practices laid out by the framework have not been observed at all.  Accordingly, these practices (i.e., collecting data, manipulating data, choosing computational tools, developing modular solutions, preparing problems, defining systems, investigating systems, and understanding relationships) are given their due attention.  Although these practices are unobserved, it is still important to discuss why they are unobserved.
...
Any manipulation of data, in its most generous sense, happens at the level of the model or algorithm that is creating the data.  Accordingly, excerpts that might generously be considered manipulating data are better classified as creating data (see Sec.~~\ref{CH5:SecCreatingData}).  This focus on the creation of data through the use of models/algorithms aligns with the learning goals of developing and using models (see P1 of Sec.~\ref{CH3:InClassWork}).

We don't expect students in ${\rm P}^{3}$ to be able to handle large of sets of data, rather, we want them to be able to correctly create those large sets of data using mathematical and computational models.

The practice of \textbf{defining systems and managing complexity} is not expected to show up in our data given that most students have very little prior programming experience.  This is possibly due to the fact that there are no computational prerequisites for ${\rm P}^{3}$.  Given this lack of prior programming experience, interaction with the system as a whole is restricted by design.  Although there is an instructor generated system that students are using (i.e., a MWP in Python that interfaces with PhysUtil and the Visual module), its complexity and management are beyond the scope of the course.  This is reflected in the absence of a learning goal (see Sec.~\ref{CH3:InClassWork}) focusing on the system as a whole.  Additionally, the problem statement itself does not explicitly require students to interact with the system as a whole, and tutors will dissuade this action.

Further, the first three MWPs all follow the same basic program structure: using a single calculation loop to integrate Newton's equations of motion with a particular force model.  Accordingly, the vast majority of time is spent working on the force model, rather than the system itself.  In this way, we limit the students' interactions with defining systems and managing their complexity.

We don't expect students in ${\rm P}^{3}$ to understand or modify the underlying system itself, rather, we just expect them to be able to use it to solve the problem.

\section{Limitations/Constraints/Considerations?}

Describe the two sections that follow.  Talk about the types of practices and how they are defined (i.e., according to the framework and within the context of the course).

\subsection{Framework}

Ultimately this should be a critique of the framework.  What the framework didn't allow us to capture.  What overlapping things it can't disentangle.  Highlight that this is based off of real data.

Linearity of characteristics.  Make sure to compare two instances (i.e., the same practice with characteristics observed in different orders).
...
The practice of \textit{creating data} involves the creation of a large set of data in an \textit{automatic} manner that is ultimately used to \textit{advance} toward some goal.  For example, when an Euler-Cromer algorithm is used to automatically generate a large set of data representing the position of the satellite over time, it can be used to advance toward the goal of generating a simulation of its realistic trajectory.  For a more detailed explanation of this example, please refer to Sec.~\ref{CH5:SecCreatingData}.

Creating data is expected to show up frequently within our data given P1 and P4 of Sec.~\ref{CH3:InClassWork}.  Specifically, students are required to develop and use models (P1) and to use math and computational thinking (P4).  Often times, the models that are being developed are subsequently being used to generate sets of data.  The various models that can (and sometimes must) be utilized are the different force models (e.g., the Newtonian gravitational force or the centripetal force) and, more generally, the Euler-Cromer model of motion.  These various models all work together to create large sets of data representing things like forces, momenta, and positions over time.

The practice of \textit{analyzing data} involves a broad process of analysis that includes sorting data into \textit{categories}, looking for \textit{trends}, looking for \textit{correlations}, and/or identifying \textit{outliers} that can be used to reach some \textit{conclusion}.  For example, when a print statement is used to verify that the force acting on the satellite has the trend of remaining constant in simulated time, a conclusion can be drawn about the correctness of the underlying force model.  For a more detailed explanation of this example, please refer to Sec.~\ref{CH5:SecAnalyzingData}.

Analyzing data is expected to show up frequently within our data given P3 and P6 of Sec.~\ref{CH3:InClassWork}.  Specifically, students are required to analyze and interpret data (P3) and to engage in argumentation from evidence (P6).  When dealing with complicated and interrelated mathematical and computational models that create large sets of data, it is important for students to be able to analyze and interpret that data, and to look for overall trends that can be used as evidence for drawing a conclusion about the underlying models. 

The practice of \textit{visualizing data} involves the \textit{production} of a visualization that clearly \textit{conveys} some information.  For example, the computational production of a dynamic graph of the distance between the satellite and the Earth vs. simulated time can be used to clearly convey information about the nature of the orbit (e.g., how close to geostationary it is).

Visualizing data is expected to show up frequently within our data given P6 and P7 of Sec.~\ref{CH3:InClassWork}.  Specifically, students are engaging in argument from evidence (P6) and obtaining, evaluating, and communicating information (P7).  Creating graphs of various physical quantities vs. time (e.g., the magnitude of the separation distance between the Earth and the satellite vs. time) and/or simulations of trajectories can be used to aid the communication of information both within and beyond a group.  In other words, these visualizations can be used as concrete evidence for argumentation.

The practice of \textit{designing computational models} involves \textit{defining} the individual components of a model, \textit{relating} the model to the physical phenomenon under investigation, and making \textit{predictions} based on the model.  For example, the mass of the satellite, the magnitude of its velocity, the radius of its orbit, and the polar angle that it makes can all be separately defined in code.  Additionally, these individual components can be combined, following an equation, to produce the expected oscillatory motion of the satellite.  Finally, the resulting force model can be used to make predictions about the motion of the satellite.  For a more detailed explanation of this example, please refer to Sec.~\ref{CH5:SecDesigingModels}.

Designing computational models is expected to show up frequently within our data given given P1 and P5 of Sec.~\ref{CH3:InClassWork}.  Specifically, students are required to develop and use models (P1) and to construct explanations (P5).  Constructing an explanation of how the model relates to the phenomenon of interest is an important part of developing the model.  Similarly, constructing an explanation of what predictions the model can be used to make is an important part of using the model.

The practice of \textit{constructing computational models} involves \textit{implementing} new behavior in code by either \textit{creating} a new model or by \textit{extending} a previously written model.  For example, implementing an attraction between two massive objects in code can be achieved through the construction of a force model.  This behavior can be implemented in one shot (e.g., immediately constructing a Newtonian gravitational force that can handle elliptical orbits) or can be implemented by successively extending an approximate model (e.g., moving from a constant gravitational force that generates a parabolic trajectory, to a centripetal force that generates a circular orbit, to a Newtonian gravitational force that generates an elliptical orbit). 

Constructing computational models is expected to show up frequently within our data given P1 and P4 of Sec.~\ref{CH3:InClassWork}.  Specifically, students are required to develop and use models (P1) and to use math and computational thinking (P4).  Constructing models in code invariably requires students to map mathematical equations onto Vpython syntax.  This involves using proper operations (e.g., adding, multiplying, calculating magnitudes), using proper order of operations (e.g., using parentheses to clear up any ambiguity), and ensuring computational abstractions are of the proper type (e.g., that position is a vector, or that distance is a scalar).

The practice of \textit{assessing computational models} involves identifying the \textit{assumptions} built into a model and \textit{validating} them by comparing to reality to ensure predictive accuracy.  For example, a uniform centripetal force model assumes that the satellite will be traveling in a perfectly circular orbit.  Although this assumption is a good starting point, it should be checked for validity when considering arbitrary initial conditions that lead to more general elliptical orbits.  For a more detailed explanation of this example, please refer to Sec.~\ref{CH5:SecAssessingModels}.

Assessing computational models is expected to show up frequently within our data given given P1 and P6 of Sec.~\ref{CH3:InClassWork}.  Specifically, students are required to develop and use models (P1) and to engage in argument from evidence (P6).  Once a model has been designed and constructed to a reasonable degree, it can be used to generate information (e.g., a trajectory of the satellite).  This information can ultimately be used as evidence to make an argument for or against the validity of that model.  Thus, throughout the process of designing, constructing, and most importantly assessing a computational model, students should be engaging in argument based on evidence.

The practice of \textit{creating computational abstractions} involves taking a physical \textit{concept} and \textit{representing} that concept in code.  For example, the physical concept of a unit vector giving the proper direction to a Newtonian gravitational force acting on the satellite can be most easily be represented in code by combining the position of the satellite and its magnitude (i.e., $\hat{r}=\vec{r}/r$).  For a more detailed explanation of this example, please refer to Sec.~\ref{CH5:SecCreatingAbstractions}.

Creating computational abstractions is expected to show up frequently within our data given P1 and P4 of Sec.~\ref{CH3:InClassWork}.  Specifically, students are required to develop and use models (P1) and to use math and computational thinking (P4).  All of the models used in the course (i.e., the various force and motion models) have some mathematical form that can be translated into VPython syntax.  That is, in order to construct a computational model, you must first create the computational abstractions that it depends on.

The practice of \textit{computer programming} involves \textit{modifying} code while \textit{arranging} that code in proper syntax.  For example, while modifying the force model in the calculation, all lines must be arranged with the proper indentation.  In other words, aside from the validity of the force model, the syntax must be in order for the computer to be able to interpret things correctly and to run without error.

Computer programming is expected to show up frequently within our data given the mere fact that groups are working with MWPs in VPython (see Sec.~\ref{CH3:MWPs}), which comes with its own unique syntax that must be adhered to strictly.  Although the syntax in VPython is very intuitive (e.g., calculating the magnitude of a vector can be done by calling the \texttt{mag()} function), small and sometimes difficult to find syntax errors (e.g., a missing parenthesis) can lead to frustrating runtime

The practice of \textit{troubleshooting and debugging} involves \textit{isolating} an unexpected error in the code, \textit{correcting} that error in a long-term and generalizable manner, and doing so in a \textit{systematic} fashion where applicable.  For example, without defining the initial momentum of the satellite as a function in terms of its previously defined mass and initial velocity, changing the mass of the satellite won't correctly propagate through the program, leading to unexpected and undesirable results.  Systematically isolating the cause of this error (i.e., not defining the momentum in a dynamic way) allows for it to not only be correct, but to be corrected in a long-term and generalizable manner.  For a more detailed explanation of this example, please refer to Sec.~\ref{CH5:SecTroubleshootingAndDebugging}.

Troubleshooting and debugging is expected to show up frequently within our data given P1 and P4 of Sec.~\ref{CH3:InClassWork}.  Specifically, students are required to develop and use models (P1) and to use math and computational thinking (P4).  While developing models, students invariable run into incorrect physical reasoning (e.g., a constant force as opposed to a position-dependent force) and invalid mathematical and computational operations (e.g., trying to divide a scalar by a vector in code).  

The practice of \textit{thinking in levels} involves breaking the MWP into different \textit{levels} and attributing those different levels with their characteristic \textit{features}.  For example, the program as a whole can broken down into the level of of initial conditions and the level of calculation loop.  Each of these levels have their own defining features: the initial conditions level is where time-independent computational abstractions can be defined, whereas the calculation loop is where time-dependent computational must be defined.  For a more detailed explanation of this example, please refer to Sec.~\ref{CH5:SecThinkingInLevels}.

Thinking in levels is expected to show up frequently within our data given P4 and P5 of Sec.~\ref{CH3:InClassWork}.  Specifically, students are required to use math and computational thinking (P4) and to construct explanations (P5).  It is important to highlight that some physical quantities (e.g., the mass of the satellite) are time-independent, whereas others (e.g., the force, position, and momentum) have an implicit time-dependence through their computational interrelation (i.e., the Euler-Cromer algorithms).  Accordingly, students should be able to construct explanations as to where computational abstractions and models, or must, be defined.

The practice of \textit{communicating information} involves the broad process of \textit{communication} that ranges from pure \textit{dialogue} to self-contained \textit{visualizations} that communicate some \textit{understanding} that the group has achieved.  For example, an understanding of the complicated but powerful computational interrelation between the force, position, and momentum of the satellite is frequently communicated verbally within and beyond groups (i.e., with TAs and LAs).  For a more detailed explanation of this example, please refer to Sec.~\ref{CH5:SecCommunicatingInformation}.

Communicating information is expected to show up frequently within our data given P5 and P7 of Sec.~\ref{CH3:InClassWork}.  Specifically, students are required to construct explanations (P5) and to obtain, evaluate, and communicate information (P7).  Ensuring that each member of a group understands each action being taken requires a continual process of explanation through dialogue.  Additionally, communication of understanding is strongly encouraged through the use of pre-defined ``tutor questions'' that each group must be able to answer (see Sec.~\ref{CH3:TutorQuestions}) before leaving for the day.

\subsection{Less common practices}

A few of the practices laid out by the framework have been identified just a couple of times and in only some of the groups that we analyzed.  Accordingly, these practices (listed in Tab.~\ref{CH6:LessCommon}) are deemed less common and are given a reasonable amount of attention.  Although these practices are less common, it is still important to clearly define them so that they can be recognized and encouraged in a classroom.

\begin{table}[hb]\centering
\begin{tabular}{llll}\hline\hline
Practice & Number & Percentage of category & Percentage of all \\\hline
Understanding concepts & 7 & 7.2 & 2.4 \\
Finding and testing solutions & 13 & 13.4 & 4.4 \\
Assessing solutions & 9 & 11.0 & 3.1 \\
Investigating systems & 13 & 13.4 & 4.4 \\
Understanding relationships & 13 & 13.4 & 4.4 \\\hline\hline
\end{tabular}
\caption{The computational practices that have been deemed less common are shown with the number of times each practice was identified, the percentage of its category that it occupies, and the percentage of all the practices that it occupies.}\label{CH6:LessCommon}
\end{table}

The practice of \textit{understanding concepts} involves \textit{progressing} toward a deeper understanding of a concept by \textit{interacting} with a computational model.  For example, while designing, constructing, or assessing a Newtonian force model in code, students progress in their understanding of the abstract concept of a unit vector as providing purely a direction to a physical quantity.

Understanding concepts is expected to show up less frequently within our data given the sheer difficulty of defining and observing understanding.  Aside from relatively rare cases where groups explicitly articulate their perceived understanding of a particular concept (see communicating information of Sec.~\ref{CH5:SecCommunicatingInformation}), this practice often goes unnoticed, regardless of whether or not understanding is actually taking place.

The practice of \textit{finding and testing solutions} involves \textit{justifying} the use of a particular solution.  For example, the information gained by using a centripetal force model with arbitrary initial conditions can be used to justify (or preclude) the use of that model.

Finding and testing solutions is expected to show up less frequently within our data given its overlap with assessing computational models (see Sec.~\ref{CH5:SecAssessingModels}).  This is especially true in this context, given that the solution coincides with a model.  In both cases, a solution/model must be justified/validated.  Aside from the relatively rare cases where the assumptions built into the model are not clearly laid out (see Sec.~4 quadrants), most instances of testing solutions are better classified as assessing models.

The practice of \textit{assessing solutions} involves the \textit{comparison} two or more different solutions.  For example, a comparison between the predicted results of a local gravitational force and a Newtonian gravitational force.

Assessing solutions is expected to show up less frequently within our data given its similarity to assessing computational models (see Sec.~\ref{CH5:SecAssessingModels}).  This is especially true in this context, given that the solution coincides with a model.  Aside from the relatively rare cases where there is an explicit comparison between two different models, most instances of assessment are better classified as assessing models.

The practice of \textit{investigating systems} involves \textit{questioning} and \textit{interpreting} data gathered from a system.  For example, the set of data representing the position of the satellite can be questioned and interpreted without knowing the underlying details of the system.

Investigating systems is expected to show up less frequently within our data given its similarity to analyzing data (see Sec.~\ref{CH5:SecAnalyzingData}).  The data representing the system as a whole usually focuses on data generated by a force or motion model and is better classified as analyzing data.

The practice of \textit{understanding relationships} involves \textit{identifying} elements of a system and \textit{explaining} their characteristic behaviors.  For example, the calculation loop can be identified and explained as being the element of the system where time-dependent quantities must be placed.

Understanding relationships is expected to show up less frequently within our data given that the structure of the minimally working program is pre-written and is explained in advance (see Sec.~\ref{CH3:MWPs}).  Additionally, given the possibility of overlap in the definition of a system and a model, this practice sometimes looks like designing computational models (see Sec.~\ref{CH5:SecDesigingModels}).  Specifically, defining the components of a system/model and relating those components to the behavior of the system/phenomenon being modeled can be viewed as either understanding relationships in a system or designing a model.

\subsection{Unobserved practices}

Some of the practices laid out by the framework have not been observed at all.  Accordingly, these practices (i.e., collecting data, manipulating data, choosing computational tools, developing modular solutions, preparing problems, defining systems, investigating systems, and understanding relationships) are given a reasonable amount of attention.  Although these practices are unobserved, it is still important to provide discussion as to why they are unobserved.

The practice of \textit{collecting data} is not expected to show up in our data given the tools that have been provided.  There are no sensors or meters that must be used to collect experimental data to analyze -- rather, the data is computationally generated using algorithms and various models (see Sec.~\ref{CH5:SecCreatingData}).

The practice of \textit{manipulating data} is also not expected to show up in our data given its focus on reshaping data (e.g., filtering or concatenating data).  Rather, any manipulation of data occurs at the model/algorithm level (see Sec.~~\ref{CH5:SecCreatingData}).

The practice of \textit{choosing computational tools} is not expected to show up in our data given the tools that have been provided.  Although other tools (e.g., Microsoft Excel) could be used to complete various portions of the task, VPython is not only the focus of the course, but is ideally suited to complete the task in its entirety.

The practice of \textit{developing modular solutions} is not expected to show up in our data given the self-contained nature of each computational problem.  If there was more overlap between different computational problems during different weeks (see Sec.~\ref{CH3:Schedule}), we might expected students to intentionally develop solutions that can be reused with slight modification.  However, the computational problems are sufficiently different to require custom solutions.

The practice of \textit{preparing problems for computational solutions} is not expected to show up in our data given that the problems have already been cast in a computational form (see Sec.~\ref{CH3:InClassWork}).  Although the problem may have a general analytic solution (i.e., a Keplerian equation of orbit) for arbitrary initial conditions, this is beyond the scope of the course.

The practice of \textit{defining systems} is not expected to show up in our data given that the structure of the minimally working program is pre-written and is explained in advance (see Sec.~\ref{CH3:MWPs}).  With most of our students being not familiar with programming, it is no surprise that this structure is left wholly unedited.