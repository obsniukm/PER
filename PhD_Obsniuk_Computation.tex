...
The computational practice of creating abstractions involves three fundamental characteristics: i) identifying computational abstractions, ii) creating computational abstractions, iii) and working toward a goal.  These three characteristics, if confidently observed within this excerpt, would serve to classify the excerpt as creating computational abstractions.

Consider Excerpt X from Group Y shown below.  Broadly, the group can be seen identifying computational abstrations, creating a computational abstraction, and using it to work toward a goal.  They start by looking at the pre-defined variables in the MPW: \begin{description}\SB \textit{(104)} yeah			
\SC \textit{(105)} its like theyve \textbf{just given} {these values}...		
\SA \textit{(106)} yeah i dunno				
\SA \textit{(107)} great question	
\end{description} Specifically, the group can be seen commenting on the fact that some of the variables in the MWP had come pre-defined with numerical values that were not realistic (line 105).  For example, the mass of the satellite was set to \texttt{1}.  They are not sure if they should be using those pre-defined and \textbf{given} values, or if they should change them to reflect the realistic situation of the mass of the satellite being \texttt{1E5}.
...
The group begins to create a computational abstraction once they have identified it:\begin{description}
...
\SB \textit{(119)} thats a good idea\end{description} Unfortunately, they are a little stuck on how to define their unit vector, but they at least know that they want it to be a function rather than just a single numerical value (line 118).  Presumably, this is because they know that the numerical values will need to change in time.

The group struggled with this for a bit: \begin{description}
