...
Conceptualizing & Some idea must be conceptualized -- either in particular and general terms.  This process of conceptualization usually involves a particular numerical value or an equation involving variables.\\
Representing & A particular idea or concept must be transformed from less general to more general.  This process of representing things more generally lies at the heart of the process of abstraction.
...
For example, consider Excerpt 2 from Group I in the following analysis.  Broadly, the group can be seen working to incorporate realistic values and generalizeable functions into their MWP.  A couple of minutes into starting the problem (Sec.~\ref{CH3:SatelliteProblem}), they modify the pre-written numerical value for the mass of the satellite from 1 to 1E4.  This leads, over the course of about thirty minutes, to the group defining the momentum of the satellite as a function.  In other words, the group can be seen \textit{isolating} and \textit{correcting} an unexpected error in a \textit{systematic way}.
