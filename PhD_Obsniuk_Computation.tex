...
In order to justify the classification of an excerpt as a particular computational practice, we started by coding the data in a systematic way.  This systematic coding process was applied to three streams of data (i.e., the side-view video, over-head video, and computer screencasts) generating three types of rational within an individual excerpt, and rationale beyond an individual excerpt).  These three types of rationale tare that were used to justify our classifications are described in detail below.

In terms of the framework, we identified the various characteristics of each group and compared them to the Weintrop framework.  Each practice, according to the framework, has any number (between one and seven) of related characteristics.  The more related characteristics that we see, the more confident we are in classifying each excerpt as a particular practice.  For example, ``identifying an unexpected error in code'' is one of the required characteristic of troubleshooting and debuggin.  Similarly, ``working to systematically rectify the unexpected error'' is clearly a related but distinct characteristic.  The identification of either of these characteristics invidivually would be hinting at the practice of troubleshooting and debugging, but both of them at the same time makes our claims stronger.  This type of rationale can be found in Column G of Fig.~\ref{CH5:Levels}.
...
In order to ensure a \textit{reliable} and reasonable justification, an iterative process of inter-rater reliability was followed.  This process of generating confidence and inter-rater reliability is described in more detail below.
