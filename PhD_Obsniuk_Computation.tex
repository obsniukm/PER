...
Although the framework gives a solid foundation and a good starting place, it does not come without its limitations.  Additionally, the course and its design invariably constrains the practices that we have and have not observed.  Furthermore, the activity itself places additional constraints on those practices.  Finally, the qualitative lenses that we have used naturally influences the analysis we have conducted.  In this section, we describe the limitations of the framework, the constraints of the course and activity, and the different lenses of our qualitative analysis.  %Finally, we discuss some of the possible improvements that can be made.

\subsection{Framework}

Although the framework that we used has many benefits, there are also some limitations that come along with it.  For the most part, these limitations are centered on the broad and sometimes vague definitions that are provided by Weintrop et. al.  Accordingly, the following sections describe those ambiguous definitions in terms of the four different categories of practices: the data, modeling, problem solving, and systems thinking practices.

\subsubsection{Data}

Within the data practices, the practice of visualizing data has a tendency to cooccur with the practice of analyzing data.  This cooccurrence might be expected given that students often analyze data by visualizing it (see Sec.~\ref{CH6:CommonPractices}).  Either way, we need to be clear about defining the fundamental difference between the two: analyzing data occurs at the level of the individual data points (e.g., looking for a pattern in a set of numerical values), whereas visualizing data occurs one level removed (e.g., visualizing the slope of a 2D graph).  Although these practices do commonly occur together, they need not, and so care must be taken to correctly identify each practice appropriately.

\subsubsection{Modeling}

Within the modeling and problem solving practices, the practice of finding and testing solutions was difficult to find within our data given its significant overlap with assessing computational models (see Sec.~\ref{CH5:SecAssessingModels}).  This is especially true within our data, given that the solution being tested is generally a model that is being assessed.  In both cases, there is some form of assessment or testing.  These two could be better distinguished by more clearly defining what is meant by a ``model.''  If that is not possible within a particular discipline, then these two really should be combined into one practice.

Additionally, understanding concepts and communicating information are expected to commonly cooccur.  This is because the understanding of a concept needs to be communicated, verbally or otherwise, in order for us to be able to observe it.  So, anytime we observe understanding concepts, we also necessarily observe the communication of information.  This overlap could be separated by more clearly defining what constitutes an ``understanding.''  If this understanding could be measured, for example, non-verbally, the two practices could be more clearly separated.

\subsubsection{Problem solving}

Within the problem solving practices, assessing solutions is difficult to confidently observe in our data given its similarity to assessing computational models.  This is especially true in our data set, given that the solution being assessed is a force model.  Aside from the cases where there is an explicit comparison between two different models (see Sec.~\ref{CH5:SecAssessingModels}), most instances of assessment are better classified as assessing models.  These two could be distinguished by more clearly defining what a ``solution'' consists of in a given discipline.  If that is not possible, then these two really should be combined into one practice.

\subsubsection{Systems}

Within the systems thinking practices, understanding relationships in a system has possibly shown up less commonly in our data given that the practice is ambiguously defined -- it can have a significant amount of overlap with the practice of designing a computational model (see Sec.~\ref{CH5:SecDesigingModels}).  This overlap ultimately depends on the ambiguous definition of a system given by Weintrop et. al.  For example, if a computational force model can be considered a system, then anytime students are designing a computational model they are also understanding the relationships in a system.  Rather, if a system refers to something more like a collection of files that are related to one other to create a program (e.g., a Python script that loads different modules/libraries), then understanding the relationships in a system would most likely not occur alongside designing a computational model.  This type of ambiguity has a tendency to lower confidence during inter-rater reliability.  Accordingly, this practice is confidently observed less often.

\subsection{Course}\label{CH6:Context}

Although the course that we collected our data from was well designed and implemented (see Sec.~\ref{CH3:Context}), it was not conducive to some of the practices laid out by the framework, stemming from the many design choices that were made early on.  These crucial design choices and their ramifications on our analysis are described in the sections that follow.

\subsubsection{Group vs. individual}

Given that the course followed a group-based approach, it was often difficult to say which individual students were actively engaging in the practice.  For example, the process of agreeing on the assumptions of the force model often involves multiple viewpoints that must be taken into account simultaneously.  Accordingly, it is difficult to ascribe the practice to any individual from the group.  However, follow-up interview data could be used as additional evidence for ascribing a particular practice to a specific individual.

\subsubsection{Scaffolding vs. discovery}

The course was reasonably scaffolded with the pre-class reading, homework, and tutor questions during class.  The pre-class reading is meant to introduce the fundamental concepts, the pre-class homework is meant to check for correct application of those concepts, and the tutor questions are meant to check for understanding of the concepts and their application while in class.  However, the frequent tutor interactions make it difficult to say whether or not the practices observed are generated by the students or by the tutors -- a ``social observer'' effect.

\subsubsection{Intro vs. advanced}\label{CH6:IntroVsAdvanced}

The course was designed around introductory physics concepts, which limited the types of force and motion models that could be analyzed.  The analysis of a more advanced classroom (e.g., computational Newtonian mechanics using higher order algorithms) may provide additional or possibly new and more sophisticated practices.

Similarly, given that many students have very little prior programming experience at the introductory level, we only see a limited set of instances of programming (from the computational problem solving practices).  However, more sophisticated classrooms requiring more complicated mathematics and computation might yield new and interesting instances of these practices of programming.

%In any classroom, there are many design decisions that either encourage or do not encourage certain practices.  For example, deciding whether to analyze an introductory physics course vs. an advanced physics course.  Or, deciding whether to have guided vs. un-guided instruction.  Or, deciding whether students should work in groups vs. individually.  There are many practices that are explicitly encouraged in ${\rm P}^{3}$, but some may not be encouraged enough (see Sec.~\ref{}).

\subsection{Activity}\label{CH6:Activity}

At the level of the activity itself, its boundaries limit the practices that we observe.  For example, the problem statement contains many direct tasks, as well as a few that are implied.  Additionally, the focus of the activity is on a relatively introductory physical concept (i.e., the gravitational force and Newton's second law).  These types of requirements and their implications on our findings are described in the sections that follow.

\subsubsection{Direct vs. implied}

Given that the problem statement contains both explicit and implied tasks, it has a large influence on the practices that we do and do not observe.  Specifically, the direct tasks in the problem statement are to i) produce a simulation of an elliptical orbit, ii) produce a diagram showing the momentum of the satellite, and iii) producing a graph of the radius of the orbit over time.  The implied tasks in the problem statement (e.g., to construct lines of code, to understand the unit vector, to communicate with each other) are often a difficult thing for students to infer and accomplish, whereas the direct tasks in the problem statement are explicitly laid out for them and so take precedence.  Making these implied tasks more direct may encourage more and newer types of practices.

Similarly, 
For example, getting satellite data from the web, which they could do, would involve other data practices -- but we did not observe this.

\subsubsection{Newtonian vs. Coulomb}

The course involved only introductory Newtonian mechanics concepts, which limits the types of force and motion models that can be analyzed.  The analysis of a more advanced physics classroom (e.g., introductory electricity and magnetism, or fluid dynamics) may provide additional or possibly new and more sophisticated practices (cf. Sec.~\ref{CH6:IntroVsAdvanced}).

\subsection{Analysis}\label{CH6:Analysis}

It is important to be clear about the various lenses we have used while analyzing our data.  First, we used a more quantitative lens from an expert standpoint by breaking the data down with a task analysis.  Next, we used a more qualitative lens as education researchers by performing a thematic analysis on the data.  These two lenses and their role in our research are described in the sections that follow.

\subsubsection{Quantitative}

The first lens that we used was that of a physics content expert, looking to ensure a rigorous treatment of introductory physics in classrooms like ${\rm P}^{3}$.  This rigorous treatment was ensured by performing a task analysis.  The task analysis (see Sec.~\ref{CH2:TaskAnalysis}) provided the necessary steps that students must take while accomplishing the goal of creating a Newtonian gravitational force model in code.  Accordingly, these necessary steps are closely related to the types of practices that we see.

For example, one of the necessary and more difficult sub-tasks for students is to construct the unit vector corresponding to the direction of the force in terms of the positions of the satellite and the Earth.  This sub-task relates to many of the practices -- most notably, the practices of constructing computational models and creating computational abstractions.  In terms of a model, the direction is just one element of the computational model representing the force, which must be constructed in code.  In terms of abstractions, the variables that represent the position of the satellite and the Earth must be identified and defined in code.  Accordingly, this sub-task is conducive to the practices of constructing models and creating abstractions -- while not as much to others.

Although this quantitative lens provides a rigorous description of the data in terms of the actions that students are engaging in, it often fails to capture a more qualitative descriptions of the student learning process (e.g., understanding the concepts).

\subsubsection{Qualitative}

The second lens was that of a qualitative education researcher, looking to ensure that computational thinking is fostered in classrooms like ${\rm P}^{}$ by way of a thematic analysis.  The various phases of the thematic analysis work to ensure the reliability of the findings (see Sec.~\ref{CH6:Findings}).  Similarly, the various questions that must be answered before conducting the analysis (see Sec.~\ref{CH2:Thematic}) ensure consistency throughout.  Naturally, these different phases and questions pertaining to the thematic analysis influence the way that we defined our practices.

For example, one of the questions that must be answered before conducting a thematic analysis is whether researchers plan to take a more inductive or theoretical approach.  In choosing a more theoretical approach, by following the Weintrop framework, we are able to better define the practices that we observe.  However, in adhering to this theoretical framework, we aren't able to capture new and unique practices.

Although this qualitative lens helps to ensure reliability and consistency in our finding, it often limits the claims that can be made.  In other words, we cannot defined new practices within introductory physics, but we can precise preliminary definitions given the framework.
