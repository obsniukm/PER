...
\item what are the computational practices common to ${\rm P}^{3}$,
\item how are those practices defined in terms of concrete examples,\
\item and why do we see those practices?\end{enumerate}
...
Although the examples that follow are meant to clearly illustrate some of the common computational practices that we have observed, they do not come without their own limitations.  Accordingly, Ch.~\ref{CH6:Discussion} provides a discussion of these limitations, as well as presents some of the less common and unobserved practices.
...
There are a number of reasons that we did not observe these practices.  Specifically, the lack of learning goals related to data collection, data manipulation, choosing tools, developing modular solutions, preparing problems for solutions, and defining systems and managing complexity.  Further, given the lack of focus on these learning goals, many tutor interactions worked to intentionally dissuade students from engaging in these unobserved practices.
...
Early research in computational physics education has suggested continuing to investigate a phenomenon called ``physics debugging \cite{Caballero2011}.''  Accordingly, a pilot study was conducted in the Fall of 2016 to better understand this phenomenon -- which ultimately raised more questions than it answered (see Ch.~\ref{CH4:Motivation}).  These additional questions motivated the need for a more rigorous and in-depth analysis of the data so that we could make and support stronger claims.

Our corpus of data consists of in-class video of nine groups working on three different computational physics problems.  This video was transcribed verbatim with gestures and actions indicated.  Similarly, overhead video and computer screencasts were collected to cross-reference with the transcript.  Given these three streams of data, we performed both a task and a thematic analysis (see Secs.~\ref{CH2:TaskAnalysis} and \ref{CH2:Thematic}) to help facilitate clear and precise definitions of the common computational practices that were observed.
