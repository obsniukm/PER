...
The practice of \textit{visualizing data} involves the \textit{production} of a visualization that clearly \textit{conveys} some information.  For example, the computational production of a dynamically updating 2D graph of the distance between the satellite and the Earth vs. simulated time can be used to clearly convey information about the nature of the orbit.  Consider the associated transcript: \begin{description}
...
\SC: Okay.
\end{description}  Here, the tutor is suggesting that they produce a graph that can be used to convey information about how close the satellite is to perfectly geostationary.

Visualizing data is expected to show up commonly in our data given that students have been working with MWPs to produce dynamic visualizations of motion since the first week of class.  The first and second computational problems, focusing on boats and hovercrafts, respectively, had to be visualized in a number of ways (e.g., producing visualizations of their trajectories or graphs of velocity vs. time).  Furthermore, the problem statement, shown in Fig.~\ref{CH3:SatelliteProblem}, explicitly asks students to generate a simulation/visualization of an elliptical orbit.  Given that the production of a visualization is both a common requirement across tasks as well as a characteristic of the practice, we expect to see this practice often.

Additionally, the tutor questions -- asked after the group has correctly constructed the Newtonian gravitational force -- invariable focus on the generation of a 2D graph to clearly show the distance between the Earth and satellite over time.  This conveying of information about the geostationary nature of the orbit is efficiently accomplished through the production of a graph.
...
The practice of \textit{manipulating data} is not expected to show up in our data given that its definition (see Sec.~\ref{CH2:Framework}) focuses on the reshaping of data (e.g., filtering a set of data or concatenating two sets of data into one).  Students are not required to reshape data in this way (e.g., by using the pandas package to merge two data sets).  Rather, they are required to create the data algorithmically, which can then be visualized or analyzed ``by eye.''

Any manipulation of data, in its most generous sense, happens at the level of the model or algorithm that is creating the data.  Accordingly, excerpts that might generously be considered manipulating data are always better classified as creating data (see Sec.~~\ref{CH5:SecCreatingData}).  This focus on creating data through the use of models/algorithms aligns with the learning goal (P1) of developing and using models for the course (see Sec.~\ref{CH3:InClassWork}).

The practice of \textit{defining systems and managing complexity} is not expected to show up in our data given that most students have very little prior programming experience.  Although there is an instructor generated system that students are using (i.e., a MWP in Python that interfaces with PhysUtil and the Visual module), its complexity and management are beyond the scope of the course.  That is, we don't expect students to understand or modify the underlying system itself, rather, we just expect them to be able to use it to model orbital trajectories.

Further, the first and second computational problems are also deterministic motion problems -- that is, the first three MWPs all follow the same basic program structure (i.e., a single while loop) with the same differential equations (i.e., Newton's equations of motion) that must be used.  Accordingly, the vast majority of time is spent working on the force model, rather than the system per se.
